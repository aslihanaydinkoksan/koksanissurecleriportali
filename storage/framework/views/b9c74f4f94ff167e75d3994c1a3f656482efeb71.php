    ...
    <div class="row mt-4">
        <div class="col-md-4">
            <div class="card border-0 shadow-sm" style="background: #f8f9fa;">
                <div class="card-body">
                    ...
                    <form action="<?php echo e(route('customers.activities.store', $customer->id)); ?>" method="POST">
                        <?php echo csrf_field(); ?>
                        <div class="mb-3">
                                    ...
                                    <?php $__empty_1 = true; $__currentLoopData = $customer->contacts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $contact): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                        <li>
                                            <div class="form-check m-1">
                                                <input class="form-check-input" type="checkbox" name="contact_persons[]" value="<?php echo e($contact->name); ?>" id="new_contact_<?php echo e($contact->id); ?>" style="cursor: pointer;">
                                                <label class="form-check-label" for="new_contact_<?php echo e($contact->id); ?>" style="cursor: pointer;"><?php echo e($contact->name); ?> <small class="text-muted">(<?php echo e($contact->title ?? 'Ünvan Yok'); ?>)</small></label>
                                            </div>
                                        </li>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="datetime-local" name="activity_date" class="form-control" value="<?php echo e(now()->format('Y-m-d\TH:i')); ?>">
                        </div>
                        <div class="mb-3">
        ...
        <div class="col-md-8">
            ...
            <div class="timeline" id="activitiesList">
                <?php $__empty_1 = true; $__currentLoopData = $customer->activities->sortByDesc('activity_date'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $activity): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <?php $isHidden = $index >= 5; ?>
                    <div class="card mb-3 border-0 shadow-sm activity-item <?php echo e($isHidden ? 'd-none load-more-act' : ''); ?>" 
                        ...
                        <div class="card-body position-relative">
                            <div class="position-absolute top-0 start-0 bottom-0 rounded-start" style="width: 5px; background: <?php echo e($activity->type == 'phone' ? '#3b82f6' : ($activity->type == 'meeting' ? '#10b981' : ($activity->type == 'email' ? '#f59e0b' : '#6b7280'))); ?>;"></div>
                            <div class="position-absolute top-0 end-0 mt-3 me-3 d-flex gap-2">
                                <button type="button" class="btn btn-sm btn-link text-primary p-0" data-bs-toggle="modal" data-bs-target="#editActivityModal<?php echo e($activity->id); ?>" title="Düzenle"><i class="fa-solid fa-pen"></i></button>
                                <form action="<?php echo e(route('customer-activities.destroy', $activity->id)); ?>" method="POST" onsubmit="return confirm('Bu kaydı silmek istediğinize emin misiniz?');">
                            ...
                            <div class="d-flex justify-content-between align-items-center mb-2 ps-2 pe-5">
                                <div>
                                    <span class="badge bg-light text-dark border me-2">
                                        <?php if($activity->type == 'phone'): ?> <i class="fas fa-phone text-primary"></i> Telefon
                                        <?php elseif($activity->type == 'meeting'): ?> <i class="fas fa-handshake text-success"></i> Toplantı
                                        <?php elseif($activity->type == 'email'): ?> <i class="fas fa-envelope text-warning"></i> E-Posta
                                        <?php elseif($activity->type == 'visit'): ?> <i class="fas fa-building text-info"></i> Ziyaret
                                        <?php else: ?> <i class="fas fa-sticky-note text-secondary"></i> Not
                                        <?php endif; ?>
                                    </span>
                                    <span class="text-muted small"><?php echo e($activity->activity_date->format('d.m.Y H:i')); ?></span>
                                </div>
                                <small class="text-muted fst-italic"><i class="fas fa-user-circle me-1"></i><?php echo e($activity->user->name ?? 'Sistem'); ?></small>
                            </div>
                            <div class="ps-2 mt-2">
                                    ...
                                    <div class="mb-2">
                                        ...
                                        <div class="mt-1">
                                            <?php $__currentLoopData = $activity->contact_persons; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $person): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                <span class="badge bg-primary bg-opacity-10 text-primary border border-primary rounded-pill px-2 py-1 me-1 mb-1"><?php echo e($person); ?></span>
                                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                        </div>
                                    </div>
                                <?php endif; ?>
                                <div class="text-dark" style="white-space: pre-line;"><?php echo e($activity->description); ?></div>
                            </div>
                        </div>
                    </div>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?> 
            ...
            <?php if($customer->activities->count() > 5): ?>
...
</div><?php /**PATH /var/www/koksan-takvim/resources/views/customers/tabs/activities.blade.php ENDPATH**/ ?>