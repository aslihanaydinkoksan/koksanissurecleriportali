        ...
        <div class="row mb-3">
            ...
            <div class="col-md-3">
                ...
                <select name="visitor_id" class="form-select" id="visitorSelect" onchange="toggleOtherVisitor(this)">
                    <option value="<?php echo e(Auth::id()); ?>">Kendim (<?php echo e(Auth::user()->name); ?>)</option>
            ...
            <tbody>
                <?php $__empty_1 = true; $__currentLoopData = $customer->visits; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $visit): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <tr class="visit-item" data-date="<?php echo e($visit->visit_date ? $visit->visit_date->format('Y-m-d') : $visit->created_at->format('Y-m-d')); ?>" data-search="<?php echo e(mb_strtolower(($visit->product->name ?? '') . ' ' . implode(' ', $visit->contact_persons ?? []) . ' ' . $visit->result . ' ' . $visit->findings)); ?>" data-status="<?php echo e($visit->visit_reason); ?>">
                        <td>
                            <?php echo e($visit->visit_date ? $visit->visit_date->format('d.m.Y H:i') : $visit->created_at->format('d.m.Y H:i')); ?>

                            <div class="mt-1">
                                <?php if($visit->visitor_name): ?>
                                    <span class="badge bg-light text-dark border small" title="Ziyaretçi">
                                        <i class="fa-solid fa-user me-1"></i> <?php echo e($visit->visitor_name); ?>

                                    </span>
                                <?php elseif($visit->visitor): ?>
                                    <span class="badge bg-light text-dark border small" title="Ziyaretçi">
                                        <i class="fa-solid fa-user me-1"></i> <?php echo e($visit->visitor->name); ?>

                                    </span>
                                <?php endif; ?>
                            </div>
                            <?php if($visit->remote_system === 'iaa'): ?>
                                <div class="mt-1">
                                    <a href="<?php echo e($visit->remote_url); ?>" target="_blank" class="badge bg-info text-white text-decoration-none" title="İAA Projesine Git">
                        ...
                        <td>
                            ...
                            <?php endif; ?>
                            <?php if($visit->lot_no): ?>
                                <?php if($visit->product): ?> <br> <?php endif; ?>
                                <small class="text-muted">Lot: <?php echo e($visit->lot_no); ?></small>
                            <?php endif; ?>
                            <?php if(!$visit->product && !$visit->lot_no): ?> - <?php endif; ?>
                        </td>
                        <td>
                            <?php $contacts = is_array($visit->contact_persons) ? $visit->contact_persons : json_decode($visit->contact_persons ?? '[]', true); ?>
                            <?php if($contacts && count($contacts) > 0): ?>
                                <?php $__currentLoopData = $contacts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <span class="badge bg-secondary bg-opacity-10 text-secondary border small"><?php echo e($p); ?></span>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            <?php else: ?> - <?php endif; ?>
                        </td>
                        ...
                        <td class="text-end">
                            <div class="d-flex justify-content-end gap-2">
                                <a href="<?php echo e(route('visits.show', $visit->id)); ?>" class="btn btn-sm btn-outline-primary" title="Detay Görüntüle"><i class="fa-solid fa-eye"></i></a>
...
<?php /**PATH /var/www/koksan-takvim/resources/views/customers/tabs/visits.blade.php ENDPATH**/ ?>