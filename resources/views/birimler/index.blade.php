...
<style>
    ...
    /* Kartlar (Tam Şeffaf) */
    .birim-card { /* Bu sayfa için özel class adı */
        border-radius: 1rem; 
    ...
    .birim-card .form-label /* (Bu sayfada label yok ama tutarlılık için) */ {
        color: #000; /* Başlıklar siyah kalsın */
    ...
    .birim-card .list-group-item {
        background-color: rgba(255, 255, 255, 0.8); /* Hafif opak arka plan */
        border-radius: 0.5rem; /* Yumuşak köşeler */
        margin-bottom: 0.5rem; /* Öğeler arası boşluk */
    ...
    .birim-card .list-group-item .btn-danger {
         font-weight: bold; /* Sil butonu yazısı kalın olsun */
     }
                ...
                <div class="card-body p-4">
                    <form method="POST" action="{{ route('birimler.store') }}">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control @error('ad') is-invalid @enderror" name="ad" placeholder="Yeni birim adı (örn: Metreküp)" required value="{{ old('ad') }}">
                    ...
                    <ul class="list-group">
                        @forelse ($birimler as $birim)
                            ...
                            <li class="list-group-item d-flex justify-content-between align-items-center shadow-sm">
                                {{ $birim->ad }}
                                <form action="{{ route('birimler.destroy', $birim->id) }}" method="POST" onsubmit="return confirm('Bu birimi silmek istediğinizden emin misiniz? Silinen birim geri alınamaz.');">
                                    @csrf
                                    @method('DELETE')
