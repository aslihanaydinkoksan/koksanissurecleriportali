        ...
        <div class="col-md-10">
            <div class="card shipment-edit-card">
                <div class="card-header d-flex justify-content-between align-items-center h4 bg-transparent border-0 pt-4">
                    ...
                    <form method="POST" action="{{ route('shipments.destroy', $shipment->id) }}" onsubmit="return confirm('Bu sevkiyat kaydını silmek istediğinizden emin misiniz?');">
                        @csrf
                        @method('DELETE')
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <select name="arac_tipi" id="arac_tipi" class="form-select @error('arac_tipi') is-invalid @enderror" required>
                                        <option value="">Seçiniz...</option>
                                        <option value="tır" @if(old('arac_tipi', $shipment->arac_tipi) == 'tır') selected @endif>Tır</option>
                                        <option value="gemi" @if(old('arac_tipi', $shipment->arac_tipi) == 'gemi') selected @endif>Gemi</option>
                                        <option value="kamyon" @if(old('arac_tipi', $shipment->arac_tipi) == 'kamyon') selected @endif>Kamyon</option>
                                    </select>
                                    @error('arac_tipi') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                </div>
                                ...
                                <div id="karaAraciAlanlari" style="display: none;">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control @error('plaka') is-invalid @enderror" id="plaka" name="plaka" value="{{ old('plaka', $shipment->plaka) }}">
                                        @error('plaka') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                    </div>
                                    <div class="mb-3" id="dorse_plakasi_div">
                                        <label for="dorse_plakasi" class="form-label">Dorse Plakası</label>
                                        <input type="text" class="form-control @error('dorse_plakasi') is-invalid @enderror" id="dorse_plakasi" name="dorse_plakasi" value="{{ old('dorse_plakasi', $shipment->dorse_plakasi) }}">
                                        @error('dorse_plakasi') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                    </div>
                                    <div class="mb-3">
                                        <label for="sofor_adi" class="form-label">Şoför Adı</label>
                                        <input type="text" class="form-control @error('sofor_adi') is-invalid @enderror" id="sofor_adi" name="sofor_adi" value="{{ old('sofor_adi', $shipment->sofor_adi) }}">
                                        @error('sofor_adi') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                    </div>
                                    <div class="mb-3">
                                        <label for="kalkis_noktasi" class="form-label">Kalkış Noktası (Gümrük/Tesis)</label>
                                        <input type="text" class="form-control @error('kalkis_noktasi') is-invalid @enderror" id="kalkis_noktasi" name="kalkis_noktasi"
                                            value="{{ old('kalkis_noktasi', $shipment->kalkis_noktasi ?? '') }}">
                                        @error('kalkis_noktasi') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                    </div>
                                    <div class="mb-3">
                                        <label for="varis_noktasi" class="form-label">Varış Noktası (Gümrük/Tesis)</label>
                                        <input type="text" class="form-control @error('varis_noktasi') is-invalid @enderror" id="varis_noktasi" name="varis_noktasi"
                                            value="{{ old('varis_noktasi', $shipment->varis_noktasi ?? '') }}">
                                        @error('varis_noktasi') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                    </div>
                                    <div class="mb-3">
                                        <label class="form-label">Sevkiyat Yönü (*)</label>
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="shipment_type" id="type_import" value="import" {{ old('shipment_type', $shipment->shipment_type) == 'import' ? 'checked' : '' }}>
                                            <label class="form-check-label" for="type_import">
                                                İthalat (Import)
                                            </label>
                                        </div>
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="shipment_type" id="type_export" value="export" {{ old('shipment_type', $shipment->shipment_type) == 'export' ? 'checked' : '' }}>
                                ...
                                <div id="gemiAlanlari" style="display: none;">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control @error('imo_numarasi') is-invalid @enderror" id="imo_numarasi" name="imo_numarasi" value="{{ old('imo_numarasi', $shipment->imo_numarasi) }}">
                                        @error('imo_numarasi') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                    </div>
                                    <div class="mb-3">
                                        <label for="gemi_adi" class="form-label">Gemi Adı</label>
                                        <input type="text" class="form-control @error('gemi_adi') is-invalid @enderror" id="gemi_adi" name="gemi_adi" value="{{ old('gemi_adi', $shipment->gemi_adi) }}">
                                        @error('gemi_adi') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                    </div>
                                    <div class="mb-3">
                                        <label for="kalkis_limani" class="form-label">Kalkış Limanı</label>
                                        <input type="text" class="form-control @error('kalkis_limani') is-invalid @enderror" id="kalkis_limani" name="kalkis_limani" value="{{ old('kalkis_limani', $shipment->kalkis_limani) }}">
                                        @error('kalkis_limani') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                    </div>
                                    <div class="mb-3">
                                        <label for="varis_limani" class="form-label">Varış Limanı</label>
                                        <input type="text" class="form-control @error('varis_limani') is-invalid @enderror" id="varis_limani" name="varis_limani" value="{{ old('varis_limani', $shipment->varis_limani) }}">
                                        @error('varis_limani') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                    </div>
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="datetime-local" class="form-control @error('cikis_tarihi') is-invalid @enderror" id="cikis_tarihi" name="cikis_tarihi" value="{{ old('cikis_tarihi', \Carbon\Carbon::parse($shipment->cikis_tarihi)->format('Y-m-d\TH:i')) }}" required>
                                    @error('cikis_tarihi') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                </div>
                                <div class="mb-3">
                                    <label for="tahmini_varis_tarihi" class="form-label">Planlanan Varış Tarihi (*)</label>
                                    <input type="datetime-local" class="form-control @error('tahmini_varis_tarihi') is-invalid @enderror" id="tahmini_varis_tarihi" name="tahmini_varis_tarihi" value="{{ old('tahmini_varis_tarihi', \Carbon\Carbon::parse($shipment->tahmini_varis_tarihi)->format('Y-m-d\TH:i')) }}" required>
                                    @error('tahmini_varis_tarihi') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                </div>
                            </div>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control @error('kargo_icerigi') is-invalid @enderror" id="kargo_icerigi" name="kargo_icerigi" value="{{ old('kargo_icerigi', $shipment->kargo_icerigi) }}" required>
                                    @error('kargo_icerigi') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                </div>
                                <div class="mb-3">
                                    <label for="kargo_tipi" class="form-label">Kargo Yük Tipi (*)</label>
                                    <select name="kargo_tipi" id="kargo_tipi" class="form-select @error('kargo_tipi') is-invalid @enderror" required>
                                        <option value="">Birim Seçiniz...</option>
                                        @foreach ($birimler as $birim)
                                        <option value="{{ $birim->ad }}" @if(old('kargo_tipi', $shipment->kargo_tipi) == $birim->ad) selected @endif>{{ $birim->ad }}</option>
                                        @endforeach
                                    </select>
                                    @error('kargo_tipi') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                </div>
                                <div class="mb-3">
                                    <label for="kargo_miktari" class="form-label">Yük Miktarı (*)</label>
                                    <input type="text" class="form-control @error('kargo_miktari') is-invalid @enderror" id="kargo_miktari" name="kargo_miktari" value="{{ old('kargo_miktari', $shipment->kargo_miktari) }}" required>
                                    @error('kargo_miktari') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                </div>

                                {{-- DOSYA YÖNETİM ALANI --}}
                                <div class="mb-3">
                                    <label for="ek_dosya" class="form-label">Yeni Ek Dosya (Mevcut Dosyayı Değiştirir)</label>
                                    <input class="form-control @error('ek_dosya') is-invalid @enderror" type="file" id="ek_dosya" name="ek_dosya">
                                    @error('ek_dosya')
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control @error('aciklamalar') is-invalid @enderror" id="aciklamalar" name="aciklamalar" rows="5">{{ old('aciklamalar', $shipment->aciklamalar) }}</textarea>
                                    @error('aciklamalar') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                </div>
                            </div>
                        </div>
