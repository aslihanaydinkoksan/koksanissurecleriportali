<div class="tab-pane fade" id="tests" role="tabpanel">
    ...
    <form action="{{ route('customers.test-results.store', $customer) }}" method="POST" autocomplete="off" class="quick-add-form" enctype="multipart/form-data">
        @csrf
        <div class="row">
        ...
        <table class="table table-sm table-striped table-hover" id="testsTable">
            ...
            <tbody>
                @forelse ($customer->testResults as $result)
                    <tr class="test-item" data-date="{{ \Carbon\Carbon::parse($result->test_date)->format('Y-m-d') }}" data-search="{{ mb_strtolower($result->test_name . ' ' . $result->summary . ' ' . ($result->product->name ?? '')) }}">
                        <td><strong>{{ $result->test_name }}</strong></td>
                        <td>
                            @if ($result->product) <span class="badge bg-primary bg-opacity-10 text-primary border border-primary">{{ $result->product->name }}</span>
                        ...
                        <td>
                            @foreach ($result->getMedia('test_reports') as $media)
                                <div class="file-list-item d-inline-flex me-2 mb-2" style="width:auto; padding: 0.2rem 0.5rem;">
                                    <span><i class="fa-solid fa-file-pdf me-2"></i>{{ Str::limit($media->file_name, 15) }}</span>
                        ...
                        <td>
                            <div class="d-flex gap-2">
                                <button type="button" class="btn btn-sm btn-outline-primary" data-bs-toggle="modal" data-bs-target="#editTestModal{{ $result->id }}" title="Düzenle"><i class="fa-solid fa-pen"></i></button>
                                <form action="{{ route('test-results.destroy', $result->id ?? 0) }}" method="POST" onsubmit="return confirm('Silmek istediğinize emin misiniz?');">
                                    @csrf @method('DELETE')
