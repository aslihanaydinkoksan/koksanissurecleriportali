<div class="tab-pane fade" id="opportunities" role="tabpanel">
    ...
    <form action="{{ route('customers.opportunities.store', $customer) }}" method="POST" class="quick-add-form mb-5">
        @csrf
        <div class="row">
            ...
            {{-- YENİ EKLENEN REKABET VE KAYIP BÖLÜMÜ --}}
            <div class="col-md-4 mb-3">
    ...
    <div class="row" id="opportunitiesList">
        @forelse($customer->opportunities as $opp)
            <div class="col-md-6 col-lg-4 mb-4 opp-item" data-date="{{ $opp->created_at->format('Y-m-d') }}"
                data-search="{{ mb_strtolower($opp->title . ' ' . ($opp->user->name ?? '') . ' ' . ($opp->competitor->name ?? '')) }}"
                data-status="{{ $opp->stage }}">
                <div class="card h-100 shadow-sm opp-card opp-{{ $opp->stage }}">
                    <div class="card-body position-relative">

                        <div class="position-absolute top-0 end-0 mt-3 me-3">
                            <form action="{{ route('opportunities.update-stage', $opp->id) }}" method="POST">
                                @csrf @method('PATCH')
                                <select name="stage"
                                    class="form-select form-select-sm border-0 bg-light fw-bold text-muted shadow-none"
                                    onchange="if(this.value === 'kaybedildi') { 
                                            this.value = '{{ $opp->stage }}'; 
                                            var modal = new bootstrap.Modal(document.getElementById('editOppModal{{ $opp->id }}')); 
                                            modal.show(); 
                                            document.getElementById('edit_opp_{{ $opp->id }}_stage').value = 'kaybedildi';
                                            toggleLossReason('edit_opp_{{ $opp->id }}_stage', 'edit_opp_{{ $opp->id }}_loss_wrapper');
                        ...
                        <div class="d-flex justify-content-between align-items-center mt-auto border-top pt-2">
                            ...
                            <div class="d-flex gap-2">
                                <button type="button" class="btn btn-sm btn-light border" data-bs-toggle="modal"
                                    data-bs-target="#historyOppModal{{ $opp->id }}" title="Tarihçeyi Gör">
                                    <i class="fa-solid fa-clock-rotate-left text-info"></i>
                                </button>
                                <button type="button" class="btn btn-sm btn-light border" data-bs-toggle="modal"
                                    data-bs-target="#editOppModal{{ $opp->id }}" title="Düzenle">
                                    <i class="fa-solid fa-pen text-primary"></i>
                                </button>
                                <form action="{{ route('opportunities.destroy', $opp->id) }}" method="POST">
                                    @csrf @method('DELETE')
